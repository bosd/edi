...
class PunchoutSession(models.Model):
    ...
    def _store_punchout_session_response(self, backend_id, response_data):
        ...
        IDS doesn't have a built-in session identifier.
        ...
        # For IDS, we need to find the most recent session for this backend
        ...
        if not session:
            _logger.error(
                "Unable to find an open IDS session for backend %s", backend_id
            )
            return False
