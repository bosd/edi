...
class PunchoutSession(models.Model):
    ...
    def _get_or_create_product_cxml(
        ...
        # Try to find by supplier product code
    ...
    def _get_uom_for_cxml_item(self, item_detail):
        """Get UoM for cXML item, using UoM mapping if configured."""
        self.ensure_one()
        uom_elem = item_detail.find("UnitOfMeasure")
        uom_code = uom_elem.text if uom_elem is not None else None

        if uom_code:
            # Try backend-specific mapping first
            mapping = self.backend_id.uom_mapping_ids.filtered(
                lambda m: m.external_code == uom_code
            )
            if mapping:
                return mapping[0].uom_id

            # Try UNECE code
            uom = self.env["uom.uom"].search([("unece_code", "=", uom_code)], limit=1)
            if uom:
                return uom
