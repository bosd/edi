...
class PunchoutSession(models.Model):
    ...
    def _get_or_create_product_ids(self, order_item):
        ...
        # Try to find by supplier product code or barcode
        domain = [
            "|",
            "&",
        ...
        if ean:
            domain = ["|"] + domain + [("barcode", "=", ean)]
        else:
            domain = domain + [("barcode", "=", False)]
    ...
    def _get_or_create_uom_ids(self, order_item):
        ...
        # Try to find UoM by IDS name or alternative names
        uom = None
        if qu_code:
            # First try backend-specific mapping
            mapping = self.backend_id.uom_mapping_ids.filtered(
                lambda m: m.external_code == qu_code
            )
            if mapping:
                uom = mapping[0].uom_id

            # Try UNECE code
            if not uom:
                uom = self.env["uom.uom"].search(
                    [("unece_code", "=", qu_code)], limit=1
                )
