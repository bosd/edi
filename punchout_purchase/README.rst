...
- [FIX] The Browse-Catalog button on the contact form no longer requires
  ...
  contact and so carry ``supplier_rank 0`` (e.g. a group parent) while
  its backend — and catalog — is perfectly usable; the button (and the
  flag) now depend on the open backend alone.

19.0.1.4.0 (2026)
-----------------

- [ADD] New ``_post_punchout_session_processed(order, new_lines)`` hook
  fired at the very end of ``process()`` (after PO create/append and
  chatter warnings). Empty in base — supplier-specific glue modules
  override it to fire follow-up enrichment, batch inquiries, ASN
  polling, etc., without monkey-patching ``process()`` itself.
